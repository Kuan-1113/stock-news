...
def _do_warrant_analyze(warrant_code: str) -> str:
    ...
    prompt = f"""你是一位台灣權證專家，請根據以下實際數據對權證進行深度分析。
**重要：請直接引用數據中的具體數值，不要使用模糊說法。若某欄位顯示「—」表示資料不足，請標明。**
...
📋 **目標權證數據解讀**
逐一說明以下指標的實際意義（直接帶入數值）：
- 到期日與剩餘天數：時間是否充裕？
- 履約價與昨收價的關係（價內/價外程度）
- Delta 值（若有）：代表股票漲1元，此權證漲多少？
- IV（若有）：隱含波動率高低對買方的影響
- 溢價率（若有）：是否合理？
- 實際槓桿（若有）：放大倍數
- 成交張數：流動性是否足夠？
