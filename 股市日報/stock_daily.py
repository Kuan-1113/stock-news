...
def analyze_single_stock(symbol: str, name: str, quote: dict, news_ctx: str = "") -> str:
    ...
    prompt = f"""資深股票分析師，根據以下實際數據分析。每項帶數值，禁空話，禁 Markdown 表格。
{stale_note}
...
以繁體中文輸出，以下八節全部完整寫出（總字數 1200 字以內），每節不得省略：

**📊 近期走勢**（2句：價格位置＋趨勢）

**🔍 技術面**
• MA5/MA20 多空排列＋數值 → 趨勢結論
• MACD：線值/訊號線/柱狀 → 動能
• RSI/KD：數值 → 超買超賣訊號
• 成交量：量比 → 量價配合

**🏦 籌碼面**（若有5日法人數據）
• 外資/投信近5日買賣超趨勢 → 法人動向判斷
• 若方向一致須特別說明

**📋 基本面**（若有數據）
• PE/PB 相對估值高低 → 合理/偏貴/便宜
• ROE/EPS/殖利率 → 獲利品質與吸引力

**📐 台指期**（台股限定，若有數據）
• 近5日正逆價差趨勢 → 市場對近期偏多/偏空
• OI 變化：增倉/減倉 → 布局方向
